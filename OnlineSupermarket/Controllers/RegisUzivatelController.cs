...
namespace OnlineSupermarket.Controllers
{
    public class RegisUzivatelController : Controller
    {
        ...
        {
            _logger.LogInformation("UserProfile POST method called");
            ...
            try
            {
                ...
                {
                    await connection.OpenAsync();
                    using (var command = new OracleCommand("AKTUALIZUJ_PROFIL_UZIVATELE", connection))
                    {
                        ...
                        if (!string.IsNullOrEmpty(model.Heslo))
                        {
                            ...
                            model.Heslo = $"{saltBase64}:{hashedPassword}";
                        }
                        ...
                        command.Parameters.Add("p_heslo", OracleDbType.Varchar2).Value = model.Heslo;

                        var result = await command.ExecuteNonQueryAsync();
                        _logger.LogInformation($"Rows affected: {result}");
                    }
                }
